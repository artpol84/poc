...
/* 
 * WARNING: to be used with https://github.com/artpol84/ucx/tree/topic/recv_mr/dummy_poll_cq ONLY
 */
...
if(0){
    volatile static int delay = 1;
    while( delay ){ 
        sleep(1);
    }
}

            debug_add_credits(window_size * writers);

            uint64_t start = rdtsc(), end;
            for(j = 0; j < window_size * writers; j++) {
                MPI_CHECK(MPI_Irecv(r_buf, 1, MPI_CHAR, MPI_ANY_SOURCE, 100, MPI_COMM_WORLD,
                        mbw_request + j));
            }
            end = rdtsc();
